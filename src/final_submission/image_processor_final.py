...
def process_image(image_path, text_prompt, color_name="red", output_path="output_image.png", box_threshold=0.3, text_threshold=0.25):
    ...
    # Load SAM model
    sam_checkpoint = get_sam_weights_path()  # Replace with actual SAM checkpoint path
    sam_model = build_sam(checkpoint=sam_checkpoint)
    sam_model.to(device)
    sam_predictor = SamPredictor(sam_model)

    # Load image and preserve original format
    original_image = Image.open(image_path)
    original_format = original_image.format
    
    # Get original image size to maintain resolution
    width, height = original_image.size
    
    image_source, image = load_image(image_path)

    default_objects = [
        "wall", "right wall", "left wall", "front wall", "ceiling", "floor"
    ]
    ...
    # Determine file extension from the output path
    file_ext = os.path.splitext(output_path)[1].lower()
    
    # If original format was JPEG and output is .jpg/.jpeg, save as JPEG with quality control
    if (original_format == 'JPEG' or file_ext in ['.jpg', '.jpeg']):
        # Preserve original format and apply compression
        result_image_pil.save(output_path, format='JPEG', quality=85, optimize=True)
    # If output is .png, use compression
    elif file_ext == '.png':
        result_image_pil.save(output_path, format='PNG', optimize=True, compress_level=6)
    else:
        # Default case, save with original format
        result_image_pil.save(output_path, optimize=True)
...
def detect_objects_and_inpaint(image_path, box_threshold=0.45, text_threshold=0.25):
    ...
    model = load_model_hf(ckpt_repo_id, ckpt_filename, ckpt_config_filename, device="cpu")
...
def detect_objects(image_path, box_threshold=0.30, text_threshold=0.20, return_objects=False):
    ...
    image = torch.from_numpy(image_np).permute(2, 0, 1).float().to("cpu")
